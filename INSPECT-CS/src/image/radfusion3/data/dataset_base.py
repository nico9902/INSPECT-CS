...
class DatasetBase(Dataset):
    def __init__(self, cfg, split="train", transform=None):
        ...
        self.split = split
        path = "/mimer/NOBACKUP/groups/naiss2023-6-336/dataset_shared/inspect2/dict_slice_thickness.pkl"
        self.dict_slice_thickness = pickle.load(open(path, "rb"))

    def init_hdf5(self):
        # called once per worker
        self.hdf5_dataset = h5py.File(self.cfg.dataset.hdf5_path, 'r', libver='latest', swmr=True)

    def __del__(self):
        if hasattr(self, 'hdf5_dataset'): self.hdf5_dataset.close()
    ...
    def read_from_hdf5(self, key, slice_idx=None):
        key = str(key)
    ...
    def read_image(self, file_path: str, slice_idx: int, resize_size=None, channels=None):
        ...
        if file_path.endswith('.nii.gz'):
            ...
            pixel_array = nifti_img.get_fdata()[:, :, slice_idx]
    ...
    def get_num_slices(self, nifti_path):
        """Helper function to get the number of slices in a NIfTI file."""
        nifti_img = nib.load(nifti_path)
        return nifti_img.shape[2]  # Assuming slices are along the third dimension

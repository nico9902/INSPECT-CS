...
def resnext_101_sup_ct(**kwargs):
    ...
    checkpoint = torch.load("/mimer/NOBACKUP/groups/naiss2023-6-336/multimodal_os/PE-Insight/experiments/model_dir/rsna_pe/resnext101_ct.ckpt")
    ckpt = {k.replace("model.", ""): v for k, v in checkpoint["state_dict"].items()}

    # Filter out classifier keys if they exist and you don't need them (aggiunto da me)

...
class Model1D(nn.Module):
    ...
    def forward(self, x, get_features=False, mask=None, return_features=False):
        x_enc = self.seq_encoder(x)
        x_agg, w = self.aggregate(x_enc, mask)
        # x_agg = self.batch_norm_layer(x_agg)
        pred = self.classifier(x_agg)
        
        if return_features:
            return pred, 0.0, {"image": x_agg.cpu().detach().numpy()}
        return pred, x_agg

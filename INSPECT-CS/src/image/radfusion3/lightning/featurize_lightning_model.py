...
class FeaturizeLightningModel(LightningModule):
        ...
        if self.features_dir:
            for ids, f in zip(instance_id, features.cpu().detach().numpy()):
                try:
                    ...
                    slice_idx = ids.split('@')[1]  # Get slice index
                    # Create the path for the feature file
                    feature_path = os.path.join(self.features_dir, f"{clean_id}_{slice_idx}.npy")
                    # Use executor to save asynchronously
                    self.executor.submit(self.save_feature_async, feature_path, f)
    ...
    def save_feature_async(self, path, feature):
    ...
    def shared_epoch_end(self, step_outputs, prefix):
        ...
        if len(all_features) > 0:
            ...
            df["slice_idx"] = df["instance_id"].apply(lambda x: x.split("@")[1])

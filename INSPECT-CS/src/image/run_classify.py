...
def run(config):
    ...
    config.exp.name = f"{config.exp.name}_{config.dataset.target}_{config.trainer.seed}" #_{strftime('%Y-%m-%d_%H:%M:%S', gmtime())}"
    ...
    # # if config.ckpt is a listConfig
    # idx = int(os.environ["SLURM_ARRAY_TASK_ID"])
    # config.ckpt = config.ckpt[idx]
    ...
    if config.ckpt is not None and config.ckpt.endswith(".ckpt"):
        ...
        if checkpoint_files:
            ...
            print(f"No checkpoint found in {save_dir}. Using provided checkpoint: {ckpt}")
            raise FileNotFoundError(f"No checkpoint found in {save_dir}. Using provided checkpoint: {ckpt}")

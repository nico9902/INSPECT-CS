...
# source_csvs_path = os.path.join(args.path_to_data, "meds_reader_omop_inspect")
...
#     os.system(
#         f"python src/ehr/1_csv_to_database.py --path_to_input {source_csvs_path} --path_to_target {database_path} --athena_download {args.path_to_athena} --num_threads 20"
#     )
...
for task in tasks:
    ...
    if not os.path.exists(task_dir):
        os.system(
            f"python src/ehr/2_generate_labels_and_features.py --path_to_cohort {cohort_path} --path_to_database {extract_path} --path_to_output_dir {task_dir} --labeling_function {task} --num_threads 4"
        )
    else:
        print(
...
#     if args.test_GBM:
#         os.system(
#             f"python src/ehr/3_train_gbm.py --path_to_cohort {cohort_path} --path_to_database {extract_path} --path_to_output_dir {task_dir} --path_to_label_features {task_labels_features_dir} --num_threads 20 --test_GBM"
#         )
...
# # 3.0. Generate custom dictionary from the extract
# custom_dictionary_path = os.path.join(args.path_to_output, "inspect_femr_extract", "dictionary")
...
#     if not os.path.exists(task_dir):
#         command = f"clmbr_create_batches {task_dir} --data_path {extract_path} --task labeled_patients --labeled_patients_path {task_labels_features_dir}/labeled_patients.csv --val_start 80 --dictionary_path {args.path_to_motor}/dictionary --is_hierarchical --transformer_vocab_size 4524"
#         os.system(command)
#     else:
...
#     if not os.path.exists(task_dir):
#         command = f"clmbr_train_linear_probe {task_dir} --path_to_cohort {task_labels_features_dir}/labeled_patients.csv --data_path {extract_path} --model_dir {args.path_to_motor}/model --batches_path {task_batches_dir} "
#         os.system(command)
#     else:
#         print(

...
if __name__ == "__main__":
    ...
    PATIENT_ID_COLUMN = "PersonID"  # "patient_id"
    ...
    mortality_codes = list(
        femr.labelers.omop.map_omop_concept_codes_to_femr_codes(
            ontology,
            ["328170"],  # aggiungi manualmente il codice di morte
            is_ontology_expansion=True,
        )
    )
    print("Mortality codes found:", mortality_codes)
    ...
    if args.labeling_function in {
        ...
        # Carica death.csv
        path_to_death_csv = "/mimer/NOBACKUP/groups/naiss2023-6-336/multimodal_os/PE-Insight/data/ehr/omop/death.csv"  # <--- Sostituisci con il path reale
        death_df = pd.read_csv(path_to_death_csv)
        death_df = death_df.dropna(subset=["death_DATE"])

        # Mappa person_id -> data di morte
        person_id_to_death_date = {
            int(row["person_id"]): datetime.datetime.fromisoformat(row["death_DATE"])
            for _, row in death_df.iterrows()
        }

        # Seleziona orizzonte temporale in base al tipo di labeling
        if args.labeling_function == "1_month_mortality":
            horizon = datetime.timedelta(days=30)
        elif args.labeling_function == "6_month_mortality":
            horizon = datetime.timedelta(days=180)
        ...
        for row in cohort_lines:
            ...
            prediction_time = row[TIME_COLUMN]

            if patient_id in person_id_to_death_date:
                death_time = person_id_to_death_date[patient_id]
                label = (death_time > prediction_time) and (death_time <= prediction_time + horizon)
            else:
                label = False

            labels[patient_id].append(
                femr.labelers.Label(time=prediction_time, value=label)
            )
